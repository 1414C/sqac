...
func (hf *HDBFlavor) AlterSequenceStart(name string, start int) error {
	...
	var nextVal int64
	valQuery := fmt.Sprintf("SELECT %s.NEXTVAL FROM dummy;", seqName)
	err = hf.db.QueryRow(valQuery).Scan(&nextVal)
	if err != nil {
		return err
	}
	return nil
...
// the current value of the named HDB identity (auto-increment) field
// in the named table.
func (hf *HDBFlavor) GetNextSequenceValue(name string) (int, error) {
	...
	var currVal int
	currQuery := fmt.Sprintf("SELECT %s.CURRVAL FROM dummy;", seqName)
